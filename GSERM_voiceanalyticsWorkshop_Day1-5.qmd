...
oscillo(cmd2, colwave="blue", title = "Second Command Wavefolisten(rm")
```
...
- `bandpass = FALSE` **removes** the band (a band-stop) — here we cut out 3–5 kHz while keeping everything else.
- `bandpass = TRUE` **keeps** the band — here we keep only 0–4 kHz, which acts as a low-pass that drops the high-frequency content.
...
w3_rmb <- fir(w3, from = 3000, to = 5000, bandpass = FALSE, output = "Wave")
```

```{r}
# Low-pass: keep <=4 kHz, drop high-frequency content
w3_lpass <- fir(w3, from = 0, to = 4000, bandpass = TRUE, output = "Wave")
```
...
trump <- readWave("voiceData/3_obama_short/trump.srt.wav")
```

We use `timer()`, which segments a recording into signal vs. pause periods from a smoothed amplitude envelope. The first speech segment's start time (`$s.start[1]`) is the onset. The `threshold` (% of max amplitude counted as "signal") and `msmooth` (envelope smoothing window) settings make this robust: smoothing keeps brief noise spikes from registering as spurious early segments.

```{r onset-detection}
obama_onset <- timer(obama, threshold = 5, plot = FALSE)$s.start[1]
trump_onset <- timer(trump, threshold = 5, plot = FALSE)$s.start[1]
```


```{r raw-onsets}
# Visualize the automatically detected speech onsets
par(mfrow = c(1, 2))
...
abline(v = trump_onset, col = "red", lwd = 2, lty = 2)
```
...
trump_cut <- extractWave(trump, from = trump_onset, to = seewave::duration(trump), xunit = "time")
``` 

```{r}
par(mfrow = c(1, 2))
oscillo(obama_cut,title="Obama Cut")
oscillo(trump_cut,title="Trump Cut")
```
...
temporal_features <- function(wave, speaker) {
  ...
  seg <- segment(as.numeric(wave@left), samplingRate = wave@samp.rate, plot = FALSE) # syllables
  ...
  tibble(
    speaker          = speaker,
    onset_s          = round(tm$s.start[1], 2),
    total_dur_s      = round(total_dur, 1),
    speech_time_s    = round(speech_time, 1),
    phonation_pct    = round(100 * speech_time / total_dur, 1),
    n_pauses         = length(tm$p),
    mean_pause_s     = round(mean(tm$p), 2),
    n_syllables      = n_syl,                          # acoustic estimate, not a true word count
    speech_rate_sps  = round(n_syl / total_dur, 2),    # syllables/s incl. pauses
    articulation_sps = round(n_syl / speech_time, 2)   # syllables/s excl. pauses
  )
}
...
```{r temporal-contrast}
feat |>
  select(speaker, onset_s, articulation_sps, mean_pause_s, n_pauses, phonation_pct) |>
  pivot_longer(-speaker, names_to = "feature", values_to = "value") |>
  mutate(feature = recode(feature,
    onset_s          = "Speech onset (s)",
    articulation_sps = "Articulation rate (syl/s)",
    mean_pause_s     = "Mean pause (s)",
    n_pauses         = "Number of pauses",
    phonation_pct    = "Phonation (% of clip)")) |>
  ggplot(aes(speaker, value)) +
  geom_col() +
  facet_wrap(~ feature, scales = "free_y") +
  labs(x = NULL, y = NULL, title = "Temporal/prosodic contrasts: Obama vs Trump")
```
